...
void CoordinateFilter::add(double longitude, double latitude, const boost::posix_time::ptime& timestep)
{
  LonLat lonlat(longitude, latitude);
...
void CoordinateFilter::add(double longitude, double latitude, double level, const boost::posix_time::ptime& timestep)
{
  LonLat lonlat(longitude, latitude);
...
{
  return (itsAllowedLevels.empty() && itsAllowedTimesteps.empty() && itsAllowedLevelsTimesteps.empty());
}
...
std::string CoordinateFilter::getDatetime() const
{
  ...
  if(!starttime.is_not_a_date_time())
	return (boost::posix_time::to_iso_extended_string(starttime)+"Z/"+boost::posix_time::to_iso_extended_string(endtime)+"Z");

  return "";
}


bool CoordinateFilter::accept(double longitude, double latitude, double level,const boost::posix_time::ptime& timestep) const
{
  if(isEmpty())
	return true;
