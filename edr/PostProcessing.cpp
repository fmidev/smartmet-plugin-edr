                ...
                TS::OutputData& outputData)
{
  try
  {
    ...
    if (std::get_if<TS::TimeSeriesPtr>(aggregatedData.data()))
    {
      TS::TimeSeriesPtr ts_first = *(std::get_if<TS::TimeSeriesPtr>(aggregatedData.data()));
      ...
      {
        TS::TimeSeriesPtr ts = *(std::get_if<TS::TimeSeriesPtr>(&data));
  ...
  try
  {
    ...
    for (const auto& output : outputData)
    {
      ...
      for (unsigned int j = 0; j < outdata.size(); j++)
      {
        ...
        {
          TS::TimeSeriesVectorPtr tsv = *(std::get_if<TS::TimeSeriesVectorPtr>(&tsdata));
