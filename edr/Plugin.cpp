                   ...
                   Spine::HTTP::Response& response)
{
  try
  {
	...
	//    std::cout << "Headers string: " << request.headersToString() << std::endl;
	auto headerss = request.getHeaders();
    std::cout << "Headers map: "  << std::endl;
	for(auto item : headerss)
	  std::cout << "head: " << item.first << " -> " << item.second << std::endl;
