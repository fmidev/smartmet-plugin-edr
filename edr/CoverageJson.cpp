...
Json::Value parse_temporal_extent(const edr_temporal_extent& temporal_extent)
{
  ...
  Json::Value get_data_queries(const EDRQuery &edr_query, const std::string& producer, bool levels_exist, bool instances_exist, const std::string& instance_id = "")
{
      ...
      if(query_type == EDRQueryType::Position)
		{
		  ...
		  query_info_link["href"] = Json::Value((edr_query.host + "/collections/" + producer + instance_str + "/position"));
      ...
      else if(query_type == EDRQueryType::Radius)
		{
		  ...
		  query_info_link["href"] = Json::Value((edr_query.host + "/collections/" + producer + instance_str + "/radius"));
      ...
      else if(query_type == EDRQueryType::Area)
		{
		  ...
		  query_info_link["href"] = Json::Value((edr_query.host + "/collections/" + producer + instance_str + "/area"));
      ...
      else if(query_type == EDRQueryType::Cube)
		{
		  ...
		  query_info_link["href"] = Json::Value((edr_query.host + "/collections/" + producer + instance_str + "/cube"));
		  ...
		  query_info_variables["bbox"] = Json::Value("BBOX value defining lower left and upper right corner coordinates i.e. Finland: bbox=19.449758 59.693749, 31.668339 70.115572");
		  query_type_string = "cube";
		}
      else if(query_type == EDRQueryType::Locations)
		{
		  query_info_link["title"] = Json::Value("Locations query");
		  query_info_link["href"] = Json::Value((edr_query.host + "/collections/" + producer + instance_str + "/locations"));
      ...
      else if(query_type == EDRQueryType::Trajectory)
		{
		  ...
		  query_info_link["href"] = Json::Value((edr_query.host + "/collections/" + producer + instance_str + "/trajectory"));
      ...
      else if(query_type == EDRQueryType::Corridor)
		{
		  ...
		  query_info_link["href"] = Json::Value((edr_query.host + "/collections/" + producer + instance_str + "/corridor"));
  ...
  if(instances_exist)
	{
      ...
      auto query_info_variables = Json::Value(Json::ValueType::objectValue);
      auto query_info_link = Json::Value(Json::ValueType::objectValue);
	  query_info_link["href"] = Json::Value((edr_query.host + "/collections/" + producer + "/instances"));
	  query_info_link["hreflang"] = Json::Value("en");
...
std::vector<TS::LonLat> get_coordinates(TS::TimeSeriesVectorPtr& tsv, const std::vector<Spine::Parameter> &query_parameters) 
{
  try 
	{
...
void add_value(const TS::TimedValue &tv, Json::Value &values_array, Json::Value &data_type, unsigned int values_index, unsigned int precision)
{
  try
    {
...
Json::Value add_prologue_one_point(boost::optional<int> level,
                                   ...
                                   int longitude_precision, int latitude_precision)
{
  try
    {
  ...
  try 
	{
	  ...
	  if(!instances_exist)
		return reportError(400, ("Collection '" + edr_query.collection_id + "' does not have instances!"));
	  ...
	  for (const auto &emd : emds) 
		{
		  ...
		  title += (" Starttime: " + boost::posix_time::to_iso_extended_string(emd.temporal_extent.start_time)+"Z");
		  title += (" Endtime: " + boost::posix_time::to_iso_extended_string(emd.temporal_extent.end_time)+"Z");
		  ...
		  instance_link_item["href"] = Json::Value((edr_query.host + "/collections/" + producer + "/" + instance_id));
		  ...
		  */
		  ...
		  instance["data_queries"] = get_data_queries(edr_query, producer, emd.vertical_extent.levels.size() > 0, false, instance_id);
		  ...
		  if(instances_exist)
			{
			  ...
			  instance_link["href"] = Json::Value((edr_query.host + "/collections/" + producer + "/instances"));
...
Json::Value format_output_data_one_point(TS::OutputData &outputData, const EDRMetaData &emd,
...
Json::Value format_coverage_collection_trajectory_alternative(const DataPerParameter& dpp, const EDRMetaData &emd, const std::vector<Spine::Parameter> &query_parameters)
{
  try 
    {
      Json::Value coverage;
  ...
  try
    {
	  ...
	  // All instances of a collection share the same locations, so just get the metadata of first instance
