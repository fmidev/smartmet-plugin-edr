...
void add_value(const TS::TimedValue& tv, Json::Value& values_array,  Json::Value& data_type, unsigned int values_index)
{
  try
	{
...
Json::Value add_prologue_one_point(boost::optional<int> level, const std::string& level_type, double longitude, double latitude)
{
  try
	{
	  Json::Value coverage;
  ...
  Json::Value add_prologue_multi_point(boost::optional<int> level, const std::string& level_type, const std::vector<TS::LonLat>& coordinates)
{
  try
	{
	  Json::Value coverage;
...
Json::Value add_prologue_coverage_collection(boost::optional<int> level, const std::string& level_type, const std::vector<TS::LonLat>& coordinates)
{
  try
	{
  ...
  try
	{
	  ...
	  link_item["href"] = Json::Value(("https://smartmet.fmi.fi/edr/collections/" + producer + "/instances"));
	  ...
	  for(const auto& emd : emds)
		{
		  ...
		  if(edr_query.query_id == EDRQueryId::SpecifiedCollectionSpecifiedInstance && instance_id != edr_query.instance_id)
			continue;

		  auto instance = Json::Value(Json::ValueType::objectValue);
		  instance["id"] = Json::Value(instance_id);
		  std::string title = ("Origintime: " + boost::posix_time::to_iso_string(emd.temporal_extent.origin_time));
		  title += (" Starttime: " + boost::posix_time::to_iso_string(emd.temporal_extent.start_time));
		  title += (" Endtime: " + boost::posix_time::to_iso_string(emd.temporal_extent.end_time));
		  ...
		  instance_link_item["href"] = Json::Value(("https://smartmet.fmi.fi/edr/collections/" + producer + "/" + instance_id));
		  ...
		  auto interval_string = (boost::posix_time::to_iso_string(emd.temporal_extent.start_time) + "/" + boost::posix_time::to_iso_string(emd.temporal_extent.end_time));
		  if(emd.temporal_extent.timestep)
			interval_string += ("/" + Fmi::to_string(*emd.temporal_extent.timestep) + "M");
...
Json::Value parse_edr_metadata(const EDRProducerMetaData& epmd, const EDRQuery& edr_query)
{
  ...
  if(edr_query.query_id == EDRQueryId::AllCollections || edr_query.query_id == EDRQueryId::SpecifiedCollection)
	return parse_edr_metadata_collections(epmd, edr_query);
  
  if(edr_query.query_id == EDRQueryId::SpecifiedCollectionAllInstances || edr_query.query_id == EDRQueryId::SpecifiedCollectionSpecifiedInstance)
		  ...
		  if(qmd.levels.size() > 1)
			{
			  ...
			  if(qmd.levels.front().type == "PressureLevel")
				  producer_emd.vertical_extent.vrs = "TODO: PARAMETRICCRS['WMO standard atmosphere layer 0',PDATUM['Mean Sea Level',ANCHOR['1013.25 hPa at 15\u00b0C']],CS[parametric,1§],AXIS['pressure (hPa)',up],PARAMETRICUNIT['Hectopascal',100.0";
			  else if(qmd.levels.front().type == "HybridLevel")				
				producer_emd.vertical_extent.vrs = "TODO: PARAMETRICCRS['WMO Hybrid Levels',CS[parametric,1],AXIS['Hybrid',up]]";
		...
		for (unsigned int j = 0; j < outdata.size(); j++)
		  {
			...
			if(i == 0)
			  {
				coverage = add_prologue_one_point(level, emd.vertical_extent.level_type,   coordinates.front().lon,  coordinates.front().lat);
										   ...
										   const std::vector<Spine::Parameter>& query_parameters)
{
				  ...
				  for(unsigned int k = 0; k < llts.timeseries.size(); k++)
					{
					  ...
					  if(i == 0)
						{
						  coverage = add_prologue_multi_point(level, emd.vertical_extent.level_type, coordinates);
						  coverage["parameters"] = get_edr_series_parameters(query_parameters, emd.parameters);
						  json_param_object["type"] = Json::Value("NdArray");
						  auto shape = Json::Value(Json::ValueType::arrayValue);
						  shape[0] = Json::Value(ts.size());
  ...
  Json::Value format_output_data_coverage_collection(TS::OutputData& outputData,
													 const EDRMetaData& emd,
													 boost::optional<int> level,
													 const std::vector<Spine::Parameter>& query_parameters)
{
				  ...
				  for(unsigned int k = 0; k < llts.timeseries.size(); k++)
					{
					  ...
					  domain_axes_x["values"][0] = Json::Value(coordinates.at(coordinate_index).lon);
					  auto domain_axes_y = Json::Value(Json::ValueType::objectValue);
					  domain_axes_y["values"] = Json::Value(Json::ValueType::arrayValue);
					  domain_axes_y["values"][0] = Json::Value(coordinates.at(coordinate_index).lat);
					  auto domain_axes_t = Json::Value(Json::ValueType::objectValue);
					  domain_axes_t["values"] = Json::Value(Json::ValueType::arrayValue);
					  domain_axes_t["values"][0] = Json::Value(boost::posix_time::to_iso_extended_string(timed_value.time.utc_time()));
					  ...
					  if(i == 0)
						{
						  coverage_collection = add_prologue_coverage_collection(level, emd.vertical_extent.level_type, coordinates);
							 ...
							 const std::vector<Spine::Parameter>& query_parameters)
{
  try
  {
	...
	if (boost::get<TS::TimeSeriesPtr>(&tsdata_first))
	  ...
	  {
		//		return format_output_data_multi_point(outputData, emd, levels, query_parameters);
		return format_output_data_coverage_collection(outputData, emd, level, query_parameters);
	  }
