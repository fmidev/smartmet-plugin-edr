...
Json::Value get_data_queries(const EDRQuery &edr_query, const std::string& producer, bool levels_exist)
{
  ...
  for(const auto& query_type : data_query_set)
    {
      ...
      if(query_type == EDRQueryType::Position)
      ...
      else if(query_type == EDRQueryType::Cube)
		{
		  ...
		  query_info_link["href"] = Json::Value((edr_query.host + "/collections/" + producer + "/cube"));
      ...
      else if(query_type == EDRQueryType::Corridor)
		{
		  ...
		  query_info_link["href"] = Json::Value((edr_query.host + "/collections/" + producer + "/corridor"));
...
void add_value(const TS::TimedValue &tv, Json::Value &values_array,
               Json::Value &data_type, unsigned int values_index, unsigned int precision)
{
  try
    {
...
Json::Value parse_edr_metadata_instances(const EDRProducerMetaData &epmd,
  ...
  try {
    ...
    for (const auto &emd : emds) {
	  ...
	  instance["data_queries"] = get_data_queries(edr_query, producer, emd.vertical_extent.levels.size() > 0);
  ...
  try
      ...
      for (const auto &item : requested_epmd)
	{
	  ...
	  */
	  ...
	  value["extent"] = extent;
	  // Optional: data_queries
	  value["data_queries"] = get_data_queries(edr_query, producer, collection_emd.vertical_extent.levels.size() > 0);
...
Json::Value
formatOutputData(TS::OutputData &outputData,
		 const EDRMetaData &emd,
		 EDRQueryType query_type,
                 const std::set<int>& levels,
		 const CoordinateFilter& coordinate_filter,
  ...
  try
    {
      ...
      if(emd.querydata.size())
	edr_md = &(emd.querydata.begin()->second.front());
      else if(emd.grid.size())
      	edr_md = &(emd.grid.begin()->second.front());
      else if(emd.observation.size())
      	edr_md = &(emd.observation.begin()->second.front());

      auto longitude_precision = 4;
      auto latitude_precision = 4;
      if(edr_md)
	{
	  longitude_precision = edr_md->getPrecision("longitude");
	  latitude_precision = edr_md->getPrecision("latitude");
	}
