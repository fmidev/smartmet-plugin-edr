...
Value::Value()
    : valueType(ValueType::nullValue), nodeKey(UNINITIALIZED_KEY),
      beginIter(data_value_vector.begin()), endIter(data_value_vector.end()) {}

Value::Value(ValueType type)
    : valueType(type), nodeKey(UNINITIALIZED_KEY),
      beginIter(data_value_vector.begin()), endIter(data_value_vector.end()) {}

Value::Value(const std::string &value)
    : valueType(ValueType::stringValue), beginIter(data_value_vector.begin()),
...
Value::Value(std::size_t value)
    : valueType(ValueType::intValue), nodeKey(UNINITIALIZED_KEY),
      beginIter(data_value_vector.begin()), endIter(data_value_vector.end()) {
  data_value = value;
}
...
Value::Value(bool value)
    : valueType(ValueType::boolValue), nodeKey(UNINITIALIZED_KEY),
      beginIter(data_value_vector.begin()), endIter(data_value_vector.end()) {
  data_value = value;
}

Value::Value(double value, unsigned int prec /*= DEFAULT_PRECISION*/)
    : valueType(ValueType::doubleValue), nodeKey(UNINITIALIZED_KEY),
      precision(prec), beginIter(data_value_vector.begin()),
      endIter(data_value_vector.end()) {
  data_value = value;
}
...
Value &Value::operator=(const Value &value) {
  if (parentNode != nullptr) {
    if (value.valueType == ValueType::objectValue) {
      ...
      //std::endl;
      auto &key_value =
          (nodeKey == UNINITIALIZED_KEY ? *this : parentNode->values[nodeKey]);
  ...
  for (const auto &item : values) {
    ...
    if (value_obj.data_value_vector.size()) {
      ...
      if (boost::algorithm::starts_with(value_array, NEWLINE))
        ...
        value.append(NEWLINE + tabs(level + 1) +
                     LEFT_SQUARE_BRACKET_PLUS_NEWLINE);
      value.append(value_array + NEWLINE + tabs(level + 1) +
    ...
    if (!result.empty() &&
        !boost::algorithm::ends_with(result, LEFT_ROUND_BRACKET_PLUS_NEWLINE) &&
...
std::string Value::data_value_vector_to_string(unsigned int level) const {
  ...
  for (const auto &dv : data_value_vector) {
    ...
    if (!value.empty()) {
      if (!ret.empty() &&
          !boost::algorithm::ends_with(ret, LEFT_ROUND_BRACKET_PLUS_NEWLINE) &&
  ...
  return (tabs(level) + LEFT_SQUARE_BRACKET_PLUS_NEWLINE + ret + NEWLINE +
          tabs(level) + RIGHT_SQUARE_BRACKET);
}
...
std::string Value::to_string_impl(unsigned int level) const {
  ...
  for (const auto &item : children) {
    ...
    children_string.append(tabs(level + 1) + "\"" + child_key +
                           "\" : " + child_value);
  }
  if (!result.empty() &&
      !boost::algorithm::ends_with(result, LEFT_ROUND_BRACKET_PLUS_NEWLINE) &&
      !boost::algorithm::ends_with(result, RIGHT_ROUND_BRACKET_PLUS_COMMA) &&
      !boost::algorithm::ends_with(result, COMMA_PLUS_NEWLINE) &&
...
std::string Value::to_string() const { return to_string_impl(0); }

std::string Value::toStyledString() const { return to_string(); }

std::string Value::value() const { return data_value.to_string(precision); }

Value::const_iterator Value::begin() const { return beginIter; }

Value::const_iterator Value::end() const { return endIter; }

void Value::append(const Value &value) {
  if (value.data_value_vector.size() > 0)
    data_value_vector.insert(data_value_vector.end(),
                             value.data_value_vector.begin(),
                             value.data_value_vector.end());
  else
    data_value_vector.push_back(value);
}

std::string
DataValue::to_string(unsigned int precision /*= DEFAULT_PRECISION*/) const {
  return (data_value_to_string(*this, precision));
}

ValueType DataValue::valueType() const { return get_value_type(*this); }
