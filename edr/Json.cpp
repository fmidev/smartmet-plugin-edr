...
Value::Value(const std::string& value) : valueType(ValueType::stringValue), beginIter(data_value_vector.begin()), endIter(data_value_vector.end())
{
  // " -> \"
...
Value::Value(const char* value) : valueType(ValueType::stringValue), beginIter(data_value_vector.begin()), endIter(data_value_vector.end())
{
  // " -> \"
  std::string string_value = value;
  boost::algorithm::replace_all(string_value, "\"", "\\\"");
...
Value::Value(std::size_t value): valueType(ValueType::intValue), nodeKey(UNINITIALIZED_KEY), beginIter(data_value_vector.begin()), endIter(data_value_vector.end())
 {
   data_value = value;
 }
  
Value::Value(int value): valueType(ValueType::intValue), nodeKey(UNINITIALIZED_KEY), beginIter(data_value_vector.begin()), endIter(data_value_vector.end())
 {
   data_value = value;
 }

Value::Value(bool value): valueType(ValueType::boolValue), nodeKey(UNINITIALIZED_KEY), beginIter(data_value_vector.begin()), endIter(data_value_vector.end())
 {
   data_value = value;
 }

Value::Value(double value, unsigned int prec /*= DEFAULT_PRECISION*/) : valueType(ValueType::doubleValue), nodeKey(UNINITIALIZED_KEY), precision(prec), beginIter(data_value_vector.begin()), endIter(data_value_vector.end())
{
  data_value = value;
}
  
Value::Value(const NullValue& value)  : valueType(ValueType::nullValue), nodeKey(UNINITIALIZED_KEY), beginIter(data_value_vector.begin()), endIter(data_value_vector.end())
{  
  data_value = value;
}
...
Value& Value::operator=(const Value& value)
{
  if(parentNode != nullptr)
	...
	{
	  // data value
	  //		  std::cout << "Assgining data value for key_0 " << std::endl;
	  auto &key_value = (nodeKey == UNINITIALIZED_KEY ? *this : parentNode->values[nodeKey]);
	  //		  std::cout << "Assgining data value for key "  << &key_value << ", "<< nodeKey  << std::endl;
	  key_value.data_value = value.data_value;
	  key_value.data_value_vector = value.data_value_vector;
	  key_value.values = value.values;
	  key_value.precision = value.precision;
    ...
    {
      //	  std::cout << "Assignment operator= to this (this, key, valuekey) " << this << ", " << nodeKey << ", "  << value.nodeKey << ", "  << value_type_to_string(valueType) << std::endl;
...
Value& Value::operator[](ArrayIndex index)
{
  if(valueType != ValueType::arrayValue)
    {
      // This is actully not error, but value type should be set before using it, for example
      //  **
