...
bool GridInterface::containsGridProducer(const Query &masterquery) {
  ...
  try {
    for (auto areaproducers = masterquery.timeproducers.begin();
         areaproducers != masterquery.timeproducers.end(); ++areaproducers) {
      for (auto producer = areaproducers->begin();
           producer != areaproducers->end(); ++producer) {
...
bool GridInterface::containsParameterWithGridProducer(
...
T::ParamLevelId GridInterface::getLevelId(const char *producerName,
                                          const Query &masterquery) {
  FUNCTION_TRACE
  try {
    ...
    if (itsGridEngine->getProducerInfoByName(producerName, producerInfo)) {
      for (auto level = masterquery.levels.rbegin();
           level != masterquery.levels.rend(); ++level) {
        T::ParamLevelId levelId = itsGridEngine->getFmiParameterLevelId(
            producerInfo.mProducerId, *level);
        if (levelId == 0) {
          // Did not find a level id. Maybe the level value is given in hPa.
          // Let's try with Pa.
          levelId = itsGridEngine->getFmiParameterLevelId(
              producerInfo.mProducerId, C_INT(*level * 100));
          if (levelId != 0)
            cnt[levelId % 20]++;
        } else {
          cnt[levelId % 20]++;
...
void GridInterface::insertFileQueries(
    QueryServer::Query &query,
    const QueryServer::QueryStreamer_sptr &queryStreamer) {
  FUNCTION_TRACE
  try {
    // Adding queries into the QueryStreamer. These queries fetch actual
    // GRIB1/GRIB2 files accoding to the file attribute definitions
    // (query.mAttributeList). The queryStreamer sends these queries to the
    // queryServer and returns the results (= GRIB files) to the HTTP client.

    for (auto param = query.mQueryParameterList.begin();
         param != query.mQueryParameterList.end(); ++param) {
      for (auto val = param->mValueList.begin(); val != param->mValueList.end();
           ++val) {
        ...
        newParam.mLocationType =
            QueryServer::QueryParameter::LocationType::Geometry;
...
bool GridInterface::isValidDefaultRequest(
  ...
  try {
    ...
    for (auto geom = defaultGeometries.begin(); geom != defaultGeometries.end();
         ++geom) {
      bool match = true;
      for (auto cList = polygonPath.begin(); cList != polygonPath.end();
           ++cList) {
        for (const auto &coordinate : *cList) {
          double grid_i = ParamValueMissing;
          double grid_j = ParamValueMissing;
          if (!Identification::gridDef
                   .getGridPointByGeometryIdAndLatLonCoordinates(
...
void GridInterface::prepareGridQuery(
    QueryServer::Query &gridQuery, AdditionalParameters &additionalParameters,
    const Query &masterquery, uint mode, int origLevelId, double origLevel,
  ...
  try {
    const char *buildIdParameters[] = {
        "origintime", "modtime", "mtime",  "level", "lat",
        "lon",        "latlon",  "lonlat", "@",     nullptr};
    ...
    T::Attribute *geomId =
        gridQuery.mAttributeList.getAttribute("grid.geometryId");
    T::Attribute *coordinateType =
        gridQuery.mAttributeList.getAttribute("contour.coordinateType");

    if (geomId != nullptr || coordinateType == nullptr) {
      gridQuery.mAttributeList.addAttribute(
          "contour.coordinateType",
          Fmi::to_string(T::CoordinateTypeValue::GRID_COORDINATES));
    }
    ...
    /*
     ...
     gridQuery.mAttributeList.setAttribute("coverType",
     Fmi::to_string(loc->covertype));
     */

    std::string timezoneName = loc->timezone;
    boost::local_time::time_zone_ptr localtz =
        itsTimezones.time_zone_from_string(loc->timezone);

    boost::local_time::time_zone_ptr tz = localtz;
    bool daylightSavingActive = false;

    boost::local_time::time_zone_ptr utctz =
        itsTimezones.time_zone_from_string("UTC");
    ...
    if (tz->has_dst())
      daylightSavingTime =
          Fmi::to_iso_string(tz->dst_local_end_time(Fmi::stoi(year)));

    // std::cout << "DAYLIGHT : " << daylightSavingTime << "\n";

    if (masterquery.toptions.mode ==
            TS::TimeSeriesGeneratorOptions::TimeSteps ||
        masterquery.toptions.mode ==
            TS::TimeSeriesGeneratorOptions::FixedTimes) {
      std::string startT = startTime;
      if (startTimeUTC) {
        boost::local_time::local_date_time localTime(toTimeStamp(startTime),
                                                     tz);
    ...
    if (grid_startTime <= daylightSavingTime &&
        grid_endTime >= daylightSavingTime) {
      daylightSavingActive = true;
      if (masterquery.toptions.timeSteps &&
          *masterquery.toptions.timeSteps != 0) {
    ...
    if (masterquery.toptions.startTimeData ||
        ...
        masterquery.toptions.mode ==
            TS::TimeSeriesGeneratorOptions::GraphTimes) {
      if (masterquery.toptions.startTimeData) {
        gridQuery.mFlags =
      ...
      if (latestTime != startTime) {
        ...
        gridQuery.mFlags =
            gridQuery.mFlags | QueryServer::Query::Flags::StartTimeNotIncluded;
      }
      ...
      while (s <= e) {
        ...
        if (masterquery.toptions.timeList.size() > 0) {
          ...
          if (masterquery.toptions.timeList.find(idx) ==
              masterquery.toptions.timeList.end())
            additionOk = false;
        }
    ...
    T::Attribute *attr =
        masterquery.attributeList.getAttribute("query.analysisTime");
    if (attr != nullptr) {
    ...
    if (attr != nullptr) {
      if (strcasecmp(attr->mValue.c_str(), "any") == 0) {
        gridQuery.mFlags = gridQuery.mFlags |
                           QueryServer::Query::Flags::AcceptNotReadyGenerations;
      }
    }
    ...
    if (masterquery.origintime) {
      if (masterquery.origintime ==
          boost::posix_time::ptime(boost::date_time::pos_infin)) {
        // Generation: latest, newest
        gridQuery.mFlags =
            gridQuery.mFlags | QueryServer::Query::Flags::LatestGeneration;
      } else if (masterquery.origintime ==
                 boost::posix_time::ptime(boost::date_time::neg_infin)) {
        // Generation: oldest
        gridQuery.mFlags =
            gridQuery.mFlags | QueryServer::Query::Flags::OldestGeneration;
      } else {
    ...
    for (auto paramfunc = masterquery.poptions.parameterFunctions().begin();
      ...
      if (loc->type == Spine::Location::Place ||
        ...
        if (loc->radius == 0)
          qParam.mLocationType =
              QueryServer::QueryParameter::LocationType::Point;
        else
          qParam.mLocationType =
              QueryServer::QueryParameter::LocationType::Circle;
      }

      if (loc->type == Spine::Location::Area ||
          loc->type == Spine::Location::BoundingBox)
        qParam.mLocationType =
            QueryServer::QueryParameter::LocationType::Polygon;

      if (loc->type == Spine::Location::Path)
        qParam.mLocationType = QueryServer::QueryParameter::LocationType::Path;

      if (sameParamAnalysisTime)
        qParam.mFlags = qParam.mFlags |
                        QueryServer::QueryParameter::Flags::SameAnalysisTime;
      ...
      if (partList.size() > 3 &&
        ...
        if (partList[1] == "1")
          qParam.mFlags |=
              QueryServer::QueryParameter::Flags::InvisibleParameter;
        ...
        if (len > 0) {
          ...
          if (partList[0] == "ISOBANDS") {
            for (size_t a = 0; a < (len - 2); a = a + 2) {
              ...
              qParam.mContourHighValues.emplace_back(
                  toDouble(list[a + 2].c_str()));
              qParam.mContourColors.emplace_back(
                  strtoll(list[a + 1].c_str(), nullptr, 16));
            }
        ...
        const char *p = pname.c_str() + partList[0].size() +
                        partList[1].size() + partList[2].size() + 3;
        qParam.mParam = p;
      } else {
        qParam.mParam = pname;
      }
      ...
      while (buildIdParameters[c] != nullptr) {
        if (strncasecmp(buildIdParameters[c], qParam.mSymbolicName.c_str(),
                        strlen(buildIdParameters[c])) == 0)
          qParam.mParameterKeyType = T::ParamKeyTypeValue::BUILD_IN;
        c++;
      }

      if (additionalParameters.isAdditionalParameter(
      ...
      for (const auto &areaproducer : areaproducers) {
        ...
        itsGridEngine->getParameterDetails(producerName, qParam.mParam,
                                           parameters);

        size_t len = parameters.size();
        if (len > 0 &&
            strcasecmp(parameters[0].mProducerName.c_str(), key.c_str()) != 0) {
        ...
        case 0:
          if (qParam.mParameterLevelId == 2) {
            ...
            qParam.mFlags =
                qParam.mFlags |
                QueryServer::QueryParameter::Flags::PressureLevelInterpolation;
          }
          break;
        ...
        case 2:
          ...
          qParam.mFlags =
              qParam.mFlags |
              QueryServer::QueryParameter::Flags::HeightLevelInterpolation;
          break;
        }
      }
...
int GridInterface::getParameterIndex(QueryServer::Query &gridQuery,
                                     const std::string &param) {
  FUNCTION_TRACE
  try {
    size_t pLen = gridQuery.mQueryParameterList.size();
    for (size_t p = 0; p < pLen; p++) {
      if (strcasecmp(gridQuery.mQueryParameterList[p].mParam.c_str(),
                     param.c_str()) == 0 ||
          strcasecmp(gridQuery.mQueryParameterList[p].mSymbolicName.c_str(),
                     param.c_str()) == 0)
        return p;
    }
    return -1;
...
void GridInterface::processGridQuery(
    const State &state, Query &masterquery, TS::OutputData &outputData,
    const QueryServer::QueryStreamer_sptr &queryStreamer,
    const AreaProducers &areaproducers,
    const ProducerDataPeriod & /* producerDataPeriod */,
    const Spine::TaggedLocation &tloc, const Spine::LocationPtr &loc,
    const std::string &country, T::GeometryId_set &geometryIdList,
    std::vector<std::vector<T::Coordinate>> &polygonPath) {
  FUNCTION_TRACE
  try {
    std::shared_ptr<ContentServer::ServiceInterface> contentServer =
    ...
    AdditionalParameters additionalParameters(
        itsTimezones, masterquery.outlocale, *masterquery.timeformatter,
    ...
    if (areaproducers.size() == 1 && masterquery.pressures.empty() &&
      ...
      if (masterquery.leveltype.empty()) {
        // The level type is empty. Maybe it is included into produces alias
        // definition.
        ...
        for (size_t t = 0; t < len; t++) {
          ...
          if (masterquery.leveltype.empty() && qLevelId < 0) {
            if (parameters[t].mLevelId > "") {
              // It seems that the preferred level id is included into the
              // producer alias definition.
    ...
    for (uint mode = 0; mode < 3; mode++) {
      ...
      case 0:
        if (masterquery.levels.empty()) {
          if (areaproducers.size() == 1 && masterquery.pressures.empty() &&
            ...
            if (qLevelId == 2) {
              itsGridEngine->getProducerParameterLevelList(producerName, 2,
                                                           0.01, tmpLevels);
              for (auto lev = tmpLevels.rbegin(); lev != tmpLevels.rend();
                   ++lev)
                levels.emplace_back(*lev);
            } else if (qLevelId == 3) {
              itsGridEngine->getProducerParameterLevelList(producerName, 3, 1,
                                                           tmpLevels);
              for (auto lev = tmpLevels.begin(); lev != tmpLevels.end(); ++lev)
                levels.emplace_back(*lev);
            } else if (masterquery.leveltype.empty()) {
              std::set<T::ParamLevelId> levelIdList;
              itsGridEngine->getProducerParameterLevelIdList(producerName,
                                                             levelIdList);
              if (levelIdList.size() == 1) {
                ...
                case 3: // model
                  itsGridEngine->getProducerParameterLevelList(producerName, 3,
                                                               1, tmpLevels);
                  for (auto lev = tmpLevels.begin(); lev != tmpLevels.end();
                       ++lev)
                    levels.emplace_back(*lev);
                  break;
                }
          ...
          if (tmpLevels.empty() && masterquery.pressures.empty() &&
              masterquery.heights.empty())
          ...
          if (qLevelId == 2) {
            ...
            for (auto level = masterquery.levels.begin();
                 level != masterquery.levels.end(); ++level)
              levels.emplace_back((double)(*level));
          }
        }
        break;
      ...
      case 1:
        aLevelId = 2;
        for (auto level = masterquery.pressures.begin();
             level != masterquery.pressures.end(); ++level)
          levels.emplace_back((double)(*level));
        break;
      ...
      for (const auto level : levels) {
        ...
        std::shared_ptr<QueryServer::Query> originalGridQuery(
            new QueryServer::Query());
        if (geometryIdStr > "")
          splitString(geometryIdStr, ',', originalGridQuery->mGeometryIdList);

        prepareGridQuery(*originalGridQuery, additionalParameters, masterquery,
                         mode, aLevelId, level, areaproducers, tloc, loc,
                         geometryIdList, polygonPath);

        std::vector<TS::TimeSeriesData> tsdatavector;
        outputData.emplace_back(
            make_pair(get_location_id(tloc.loc), tsdatavector));

        if (queryStreamer != nullptr) {
          originalGridQuery->mFlags |=
              QueryServer::Query::Flags::GeometryHitNotRequired;
        }

        // gridQuery->print(std::cout,0,0);

        // Executing the query. The reqsult query object is returned as a shared
        // pointer to a Query-object. This object can point to the original
        // query-object as well as a cached query-object.
          ...
          for (uint x = 0; x < xLen; x++) {
            if ((gridQuery->mQueryParameterList[p].mValueList[x]->mFlags &
              ...
              auto dt = boost::posix_time::from_time_t(
                  gridQuery->mQueryParameterList[p]
                      .mValueList[x]
                      ->mForecastTimeUTC);
            ...
            if (coordinates.empty()) {
              uint len = gridQuery->mQueryParameterList[p]
                             .mValueList[x]
                             ->mValueList.getLength();
              if (len > 0) {
        ...
        for (int pp = 0; pp < pLen; pp++) {
          ...
          if ((gridQuery->mQueryParameterList[pp].mFlags &
            ...
            TS::TimeSeriesPtr tsForParameter(
                new TS::TimeSeries(state.getLocalTimePool()));
            TS::TimeSeriesPtr tsForNonGridParam(
                new TS::TimeSeries(state.getLocalTimePool()));
            TS::TimeSeriesGroupPtr tsForGroup(new TS::TimeSeriesGroup());

            // Counting the number of values that the parameter can have in
            // single timestep.
            ...
            if (C_INT(gridQuery->mQueryParameterList.size()) > pp &&
              ...
              uint timestepCount =
                  gridQuery->mQueryParameterList[pp].mValueList.size();
              for (uint x = 0; x < timestepCount; x++) {
                if (gridQuery->mQueryParameterList[pp]
                        .mValueList[x]
                        ->mValueData.size() > 0)
                  xLen = 1;

                uint vv = gridQuery->mQueryParameterList[pp]
                              .mValueList[x]
                              ->mValueList.getLength();
                if (vv > vLen) {
                  vLen = vv;
            ...
            if (vLen == 0 && ai > 0) {
              ...
              if (C_INT(gridQuery->mQueryParameterList.size()) > pid &&
                ...
                uint timestepCount =
                    gridQuery->mQueryParameterList[pid].mValueList.size();
                for (uint x = 0; x < timestepCount; x++) {
                  ...
                  uint vv = gridQuery->mQueryParameterList[pid]
                                .mValueList[x]
                                ->mValueList.getLength();
                  if (vv > vLen) {
                    vLen = vv;
            ...
            if (vLen > 0 && rLen <= tLen) {
              ...
              for (uint v = 0; v < vLen; v++) {
                ...
                for (uint t = 0; t < tLen; t++) {
                  auto dt = boost::posix_time::from_time_t(
                      gridQuery->mQueryParameterList[pid]
                          .mValueList[t]
                          ->mForecastTimeUTC);
                  boost::local_time::local_date_time queryTime(dt, tz);
                  ...
                  bool res = gridQuery->mQueryParameterList[pid]
                                 .mValueList[t]
                                 ->mValueList.getGridValueByIndex(v, val);
                  if (res && (val.mValue != ParamValueMissing ||
                              val.mValueString.length() > 0)) {
                    pidList.insert(std::pair<ulonglong, uint>(
                        ((ulonglong)pIdx << 32) + t, pp));
                  } else if (ai > 0) {
                ...
                if (vLen > 1) {
                  ...
                  if (gridQuery->mQueryParameterList[pid]
                          .mValueList[0]
                          ->mValueList.getGridValueByIndex(v, val)) {
                    TS::LonLatTimeSeries llSeries(TS::LonLat(val.mX, val.mY),
                                                  ts);
            ...
            if (!processed && coordinates.size() > 1) {
              const char *cname[] = {"lat",    "latitude", "lon",  "longitude",
                                     "latlon", "lonlat",   nullptr};
              uint aaa = 0;
              while (
                  cname[aaa] != nullptr &&
                  strcasecmp(gridQuery->mQueryParameterList[pid].mParam.c_str(),
                             cname[aaa]) != 0)
                aaa++;
            ...
            if (!processed) {
              // The query has returned no values for the parameter. This
              // usually means that the parameter is not a data parameter. It is
              // most likely "a special parameter" that is based on the given
              ...
              for (auto ft = gridQuery->mForecastTimeList.begin();
                   ft != gridQuery->mForecastTimeList.end(); ++ft) {
                auto dt = boost::posix_time::from_time_t(*ft);
                boost::local_time::local_date_time queryTime(dt, tz);

                if (xLen == 1) {
                  size_t size = gridQuery->mQueryParameterList[pid]
                                    .mValueList[t]
                                    ->mValueData.size();
                  uint step = 1;
                  if (size > 0)
                  ...
                  int width =
                      Fmi::stoi(gridQuery->mAttributeList.getAttributeValue(
                          "grid.width"));
                  int height =
                      Fmi::stoi(gridQuery->mAttributeList.getAttributeValue(
                          "grid.height"));
                  bool rotate = (bool)Fmi::stoi(
                      gridQuery->mAttributeList.getAttributeValue(
                  ...
                  if (width > 0 && height > 0) {
                    ...
                    ImagePaint imagePaint(imageWidth, imageHeight, 0xFFFFFFFF,
                                          0x000000, 0xFFFF00, false, !rotate);

                    for (size_t s = 0; s < size; s++) {
                      uint col = 0x000000;
                      // printf("--- colors %d = %lu
                      // %lu\n",p,s,gridQuery->mQueryParameterList[pid].mContourColors.size());
                      if (s < gridQuery->mQueryParameterList[pid]
                                  .mContourColors.size()) {
                        col = gridQuery->mQueryParameterList[pid]
                                  .mContourColors[s];
                      } else {
                        uint r = s * step;
                        col = (r << 16) + (r << 8) + r;
                    ...
                    if (cpid != pidList.end()) {
                      ...
                      if ((gridQuery->mQueryParameterList[pid].mFlags &
                           QueryServer::QueryParameter::Flags::
                               InvisibleParameter) == 0) {
                        std::string s1 =
                            R"(<img border="5" src="data:image/png;base64,)";
                    ...
                    if ((gridQuery->mQueryParameterList[pid].mFlags &
                      ...
                      tsForNonGridParam->emplace_back(tsValue);
                            ...
                            QueryServer::QueryParameter::Flags::
                                InvisibleParameter) != 0) {
                } else if (strcasecmp(gridQuery->mQueryParameterList[pid]
                                          .mParam.c_str(),
                                      "y") == 0) {
                  uint len = coordinates.size();
                  ...
                  if (masterquery.crs.empty() ||
                      masterquery.crs == "EPSG:4326") {
                    for (uint i = 0; i < len; i++) {
                  ...
                  tsForNonGridParam->emplace_back(tsValue);
                               ...
                               tloc.tag, loc, country,
                               gridQuery->mQueryParameterList[pid].mPrecision,
                               paramValue)) {
                  TS::TimedValue tsValue(queryTime, paramValue);
                  tsForNonGridParam->emplace_back(tsValue);
                } else if (additionalParameters
                               .getParameterValueByLocationAndTime(
                                   gridQuery->mQueryParameterList[pid].mParam,
                                   tloc.tag, loc, queryTime, tz,
                                   gridQuery->mQueryParameterList[pid]
                                       .mPrecision,
                                   paramValue)) {
                  TS::TimedValue tsValue(queryTime, paramValue);
                  tsForNonGridParam->emplace_back(tsValue);
                } else if (gridQuery->mQueryParameterList[pid].mParam.substr(
                               0, 3) == "@L-") {
                  auto opt_idx = Fmi::stoi_opt(
                      gridQuery->mQueryParameterList[pid].mParam.substr(3));
                ...
                } else if (gridQuery->mQueryParameterList[pid].mParam ==
                           "level") {
                  int idx = 0;
                  ...
                  while (idx < pLen && levelValue <= 0) {
                    if (gridQuery->mQueryParameterList[idx]
                            .mValueList[t]
                            ->mParameterLevel > 0) {
                      levelValue = gridQuery->mQueryParameterList[idx]
                                       .mValueList[t]
                                       ->mParameterLevel;
                      if (levelValue < 0)
                        if (gridQuery->mQueryParameterList[idx]
                  ...
                  tsForNonGridParam->emplace_back(tsValue);
                               ...
                               0, 4) == "@LT-") {
                  auto opt_idx = Fmi::stoi_opt(
                      gridQuery->mQueryParameterList[pid].mParam.substr(4));
                  if (opt_idx && *opt_idx >= 0 && *opt_idx < pLen) {
                    auto cpid = pidList.find(((ulonglong)(*opt_idx) << 32) + t);
                    if (cpid != pidList.end()) {
                      TS::TimedValue tsValue(
                  ...
                  while (idx < pLen) {
                    if (gridQuery->mQueryParameterList[idx]
                      ...
                      if (itsGridEngine->getProducerInfoById(
                              gridQuery->mQueryParameterList[idx]
                                  .mValueList[t]
                                  ->mProducerId,
                              producer)) {
                  ...
                  if (idx == pLen) {
                    ...
                    if (gridQuery->mProducerNameList.size() == 1) {
                      std::vector<std::string> pnameList;
                      itsGridEngine->getProducerNameList(
                          gridQuery->mProducerNameList[0], pnameList);
                      if (pnameList.size() > 0)
                    ...
                    if (gridQuery->mQueryParameterList[idx]
                      ...
                      if (res) {
                        ...
                        boost::local_time::local_date_time origTime(
                            toTimeStamp(info.mAnalysisTime), tz);
                  ...
                  while (idx < pLen) {
                    if (gridQuery->mQueryParameterList[idx]
                      ...
                      TS::TimedValue tsValue(
                          queryTime,
                          masterquery.timeformatter->format(modTime));
                      tsForNonGridParam->emplace_back(tsValue);
                      idx = pLen + 10;
                    }
                    idx++;
                  ...
                  if (opt_idx && *opt_idx >= 0 && *opt_idx < pLen) {
                    ...
                    if (cpid != pidList.end()) {
                      ...
                      T::ProducerInfo producer;
                      if (itsGridEngine->getProducerInfoById(
                              gridQuery->mQueryParameterList[i]
                                  .mValueList[t]
                                  ->mProducerId,
                              producer))
                      ...
                      sprintf(tmp, "%s:%s:%u:%d:%d:%d:%d",
                              ...
                              gridQuery->mQueryParameterList[i]
                                  .mValueList[t]
                                  ->mGeometryId,
                              C_INT(gridQuery->mQueryParameterList[i]
                                        .mValueList[t]
                                        ->mParameterLevelId),
                              C_INT(gridQuery->mQueryParameterList[i]
                                        .mValueList[t]
                    ...
                    (gridQuery->mQueryParameterList[pid].mFlags &
                  ...
                  sprintf(tmp, "%s:%s:%u:%d:%d:%d:%d %s flags:%d",
                          ...
                          gridQuery->mQueryParameterList[pid]
                              .mValueList[t]
                              ->mGeometryId,
                          C_INT(gridQuery->mQueryParameterList[pid]
                                    .mValueList[t]
                                    ->mParameterLevelId),
                          C_INT(gridQuery->mQueryParameterList[pid]
                                    .mValueList[t]
                  ...
                  if (opt_idx && *opt_idx >= 0 && *opt_idx < pLen) {
                    ...
                    if (cpid != pidList.end()) {
                      ...
                      TS::TimedValue tsValue(
                          queryTime,
                          Fmi::to_string(gridQuery->mQueryParameterList[i]
                                             .mValueList[t]
                                             ->mGeometryId));
                      tsForNonGridParam->emplace_back(tsValue);
                    }
                  ...
                  splitString(gridQuery->mQueryParameterList[pid].mParam, '-',
                              partList);
                  ...
                  for (uint pl = 1; pl < partCount; pl++) {
                    ...
                    int i = pidList[idx];
                    sprintf(filename,
                            "/tmp/timeseries_contours_%llu_%d_%lu.png",
                            outputTime, i, t);
                  ...
                  // It seems that there are more values available for each time
                  // step than expected. This usually happens if the requested
                  // parameter definition does not contain enough information in
                  // order to identify an unique parameter. For example, if the
              ...
              if (tsForNonGridParam->size() > 0) {
                aggregatedData.emplace_back(erase_redundant_timesteps(
                    TS::aggregate(tsForNonGridParam,
            ...
            if (tsForParameter->size() > 0) {
              aggregatedData.emplace_back(erase_redundant_timesteps(
                  TS::aggregate(tsForParameter, paramFuncs[pIdx].functions),
                  aggregationTimes));
            }

            if (tsForGroup->size() > 0) {
              aggregatedData.emplace_back(erase_redundant_timesteps(
                  TS::aggregate(tsForGroup, paramFuncs[pIdx].functions),
                  aggregationTimes));
            }

            UtilityFunctions::store_data(aggregatedData, masterquery,
                                         outputData);
            pIdx++;
          }
        }
