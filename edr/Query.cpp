...
void report_unsupported_option(const std::string &name,
                               const boost::optional<std::string> &value) {
  if (value)
    std::cerr << (Spine::log_time_str() + ANSI_FG_RED +
                  " [timeseries] Deprecated option '" + name + ANSI_FG_DEFAULT)
              << std::endl;
}

Fmi::ValueFormatterParam
valueformatter_params(const Spine::HTTP::Request &req) {
  Fmi::ValueFormatterParam opt;
  opt.missingText =
      Spine::optional_string(req.getParameter("missingtext"), "nan");
  opt.floatField =
      Spine::optional_string(req.getParameter("floatfield"), "fixed");
...
std::string transform_coordinates(const std::string &wkt,
  ...
  try {
    ...
    while (iter != wkt.end()) {
      ...
      if (!transformation.transform(lon_coord, lat_coord))
        throw Fmi::Exception::Trace(
            BCP, "Failed to transform coordinates to WGS84: " + lon_str + ", " +
                     lat_str);
    ...
    throw Fmi::Exception::Trace(
        BCP, "Failed to transform coordinates to WGS84: " + crs + ", " + wkt);
  }
}
...
Query::Query(const State &state, const Spine::HTTP::Request &request,
             Config &config)
  ...
  try {
    ...
    boost::algorithm::split(resource_parts, resource,
    ...
    if (resource_parts.size() > 0) {
      if (resource_parts.at(0) == "edr") {
        if (resource_parts.size() > 1 &&
            resource_parts.at(1) == "collections") {
                ...
                if (resource_parts.size() > 4 &&
                  ...
                  if (resource_parts.size() > 5 &&
                    ...
                    if (itsEDRQuery.query_type == EDRQueryType::Locations)
                      itsEDRQuery.query_id =
                          EDRQueryId::SpecifiedCollectionLocations;
                  }
                }
              } else {
    ...
    std::set<EDRQueryType> supportedQueries =
        itsEDRQuery.data_queries.find(itsEDRQuery.collection_id) !=
                itsEDRQuery.data_queries.end()
    ...
    if (itsEDRQuery.query_id == EDRQueryId::DataQuery &&
        supportedQueries.find(itsEDRQuery.query_type) ==
            supportedQueries.end()) {
      throw Fmi::Exception(
          BCP, "Invalid query type '" + to_string(itsEDRQuery.query_type) +
    ...
    bool grid_producer =
        (itsEDRQuery.collection_id.find(".") != std::string::npos);
    if (!grid_producer)
    ...
    if ((itsEDRQuery.query_type == EDRQueryType::Position ||
        ...
        coords.empty())
      throw Fmi::Exception(
          BCP, "Query parameter 'coords' must be defined for Position, Radius, "
    ...
    if (!coords.empty()) {
      ...
      auto within_units =
          Spine::optional_string(req.getParameter("within-units"), "");
      if (itsEDRQuery.query_type == EDRQueryType::Radius &&
          (within.empty() || within_units.empty()))
        throw Fmi::Exception(BCP, "Query parameter 'within' and 'within-units' "
                                  "must be defined for Radius query");

      if (itsEDRQuery.query_type == EDRQueryType::Corridor) {
        auto corridor_width =
            Spine::optional_string(req.getParameter("corridor-width"), "");
        auto width_units =
            Spine::optional_string(req.getParameter("width-units"), "");
        if (corridor_width.empty() || width_units.empty())
          throw Fmi::Exception(
              BCP, "Query parameter 'corridor-width' and 'width-units' must be "
      ...
      if ((itsEDRQuery.query_type == EDRQueryType::Radius ||
        ...
        if (within_units == "mi")
          ...
          throw Fmi::Exception(BCP, "Invalid within-units option '" +
                                        within_units +
      ...
      if (itsEDRQuery.query_type == EDRQueryType::Position) {
        ...
        if (geometry_name == "MULTIPOINTZ") {
          ...
          boost::algorithm::split(coordinates, geometry_values,
                                  boost::algorithm::is_any_of(","));
          ...
          for (auto &coordinate_item : coordinates) {
            ...
            if (coordinate_item.front() == '(' && coordinate_item.back() == ')')
              coordinate_item =
                  coordinate_item.substr(1, coordinate_item.length() - 2);
            std::vector<string> parts;
            boost::algorithm::split(parts, coordinate_item,
                                    boost::algorithm::is_any_of(" "));
            if (parts.size() != 3)
            ...
            itsCoordinateFilter.add(Fmi::stod(parts[0]), Fmi::stod(parts[1]),
                                    Fmi::stod(parts[2]));
          }
      ...
      if (itsEDRQuery.query_type == EDRQueryType::Trajectory ||
        ...
        if (geometry_name == "LINESTRINGZM") {
          ...
          for (const auto &coordinate_item : coordinates) {
            ...
            if (parts.size() != 4)
              throw Fmi::Exception(
                  BCP, "Invalid LINESTRINGZM definition, longitude, latitude, "
                       "level, epoch time expected: " +
                           coords);
            wkt.append(parts[0] + " " + parts[1] + ",");
            itsCoordinateFilter.add(
                Fmi::stod(parts[0]), Fmi::stod(parts[1]), Fmi::stod(parts[2]),
          ...
          for (const auto &item : coordinates) {
            ...
            wkt.append(parts[0] + " " + parts[1] + ",");
            itsCoordinateFilter.add(
                Fmi::stod(parts[0]), Fmi::stod(parts[1]),
      ...
      if (!crs.empty() && crs != "EPSG:4326" && crs != "WGS84")
        throw Fmi::Exception(BCP, "Invalid crs: " + crs +
      ...
      */
      ...
      if (itsEDRQuery.query_type == EDRQueryType::Locations) {
        if (resource_parts.size() != 5)
          throw Fmi::Exception(
              BCP, "Missing 'locationId' in Locations query! Format "
        ...
        if (!emd.locations)
          throw Fmi::Exception(
              BCP, "Location query is not supported by collection '" +
                       itsEDRQuery.collection_id + "'!");

        if (emd.locations->find(location_id) == emd.locations->end())
          throw Fmi::Exception(
              BCP, "Invalid locationId '" + location_id + "' for collection '" +
                       itsEDRQuery.collection_id +
        ...
        if (bbox.empty())
          throw Fmi::Exception(
              BCP, "Query parameter 'bbox' must be defined for Cube!");
        ...
        parts.clear();
        boost::algorithm::split(parts, lower_left_coord,
                                boost::algorithm::is_any_of(" "));
        if (parts.size() != 2)
          throw Fmi::Exception(
              BCP,
        ...
        parts.clear();
        boost::algorithm::split(parts, upper_right_coord,
                                boost::algorithm::is_any_of(" "));
        if (parts.size() != 2)
          throw Fmi::Exception(
              BCP,
        ...
        auto wkt =
            ("POLYGON((" + lower_left_x + " " + lower_left_y + "," +
             lower_left_x + " " + upper_right_y + "," + upper_right_x + " " +
             upper_right_y + "," + upper_right_x + " " + lower_left_y + "," +
    ...
    if (datetime.find("/") != std::string::npos) {
      ...
      boost::algorithm::split(datetime_parts, datetime,
                              boost::algorithm::is_any_of("/"));
    ...
    auto parameter_names =
        Spine::optional_string(req.getParameter("parameter-name"), "");

    // Check valid parameters for requested producer
    std::list<string> param_names;
    boost::algorithm::split(param_names, parameter_names,
                            boost::algorithm::is_any_of(","));

    if (emd.vertical_extent.levels.empty() &&
        itsEDRQuery.query_type == EDRQueryType::Cube)
      throw Fmi::Exception(BCP, "Error! Cube query not possible for '" +
                                    itsEDRQuery.collection_id +
    ...
    if (grid_producer) {
      ...
      boost::algorithm::split(producer_parts, itsEDRQuery.collection_id,
                              boost::algorithm::is_any_of("."));
    ...
    if (parameter_names.empty()) {
      if (emd.parameters.empty())
        throw Fmi::Exception(BCP, "No metadata for " +
                                      itsEDRQuery.collection_id + "!");
      else
    ...
    language = Spine::optional_string(req.getParameter("lang"),
    ...
    if (!attr.empty()) {
      ...
      while (ind) {
        ...
        if (loopCount > 10)
          throw Fmi::Exception(
              BCP, "The alias definitions seem to contain an eternal loop!");
    ...
    T::Attribute *v1 = attributeList.getAttributeByNameEnd(
        "Grib1.IndicatorSection.EditionNumber");
    T::Attribute *v2 = attributeList.getAttributeByNameEnd(
        "Grib2.IndicatorSection.EditionNumber");

    T::Attribute *lat =
        attributeList.getAttributeByNameEnd("LatitudeOfFirstGridPoint");
    T::Attribute *lon =
        attributeList.getAttributeByNameEnd("LongitudeOfFirstGridPoint");
    ...
    if (v1 != nullptr && lat != nullptr && lon != nullptr) {
      ...
      loptions.reset(new Engine::Geonames::LocationOptions(
      ...
      auto lon_coord =
          Spine::optional_double(req.getParameter("x"), kFloatMissing);
      auto lat_coord =
          Spine::optional_double(req.getParameter("y"), kFloatMissing);
      auto source_crs = Spine::optional_string(req.getParameter("crs"), "");

      if (lon_coord != kFloatMissing && lat_coord != kFloatMissing &&
          !source_crs.empty()) {
        ...
        auto tmpReq = req;
        tmpReq.addParameter("lonlat", (Fmi::to_string(lon_coord) + "," +
                                       Fmi::to_string(lat_coord)));
        loptions.reset(new Engine::Geonames::LocationOptions(
            state.getGeoEngine().parseLocations(tmpReq)));
      }
    }
    ...
    starttimeOptionGiven =
        (!!req.getParameter("starttime") || !!req.getParameter("now"));
    std::string endtime =
        Spine::optional_string(req.getParameter("endtime"), "");
    endtimeOptionGiven = (endtime != "now");

#ifndef WITHOUT_OBSERVATION
    latestObservation = (endtime == "now");
    allplaces =
        (Spine::optional_string(req.getParameter("places"), "") == "all");
#endif
    ...
    timeformat = Spine::optional_string(req.getParameter("timeformat"),
                                        config.defaultTimeFormat());

    maxdistanceOptionGiven = !!req.getParameter("maxdistance");
    maxdistance = Spine::optional_string(req.getParameter("maxdistance"),
                                         config.defaultMaxDistance());

    findnearestvalidpoint =
        Spine::optional_bool(req.getParameter("findnearestvalid"), false);
    ...
    // observation params
    numberofstations =
        Spine::optional_int(req.getParameter("numberofstations"), 1);
#endif
    ...
    if (name) {
      ...
      vector<string> parts;
      boost::algorithm::split(parts, fmisidreq,
                              boost::algorithm::is_any_of(","));
    ...
    std::string dataCache =
        Spine::optional_string(req.getParameter("useDataCache"), "true");
    ...
    if (!!req.getParameter("weekday")) {
      ...
      vector<string> parts;
      boost::algorithm::split(parts, sweekdays,
                              boost::algorithm::is_any_of(","));
    ...
    throw Fmi::Exception::Trace(
        BCP, "TimeSeries plugin failed to parse query string options!")
        .disableStackTrace();
  }
}
...
#endif
{
  try {
    ...
    if (!opt.empty())
      boost::algorithm::split(resultProducers, opt,
                              boost::algorithm::is_any_of(";"));
    else if (!opt2.empty())
      boost::algorithm::split(resultProducers, opt2,
                              boost::algorithm::is_any_of(";"));
    ...
    for (const auto &tproducers : resultProducers) {
      ...
      boost::algorithm::split(producers, tproducers,
                              boost::algorithm::is_any_of(","));
...
void Query::parse_precision(const Spine::HTTP::Request &req,
                            const Config &config) {
  try {
    string precname = Spine::optional_string(req.getParameter("precision"),
    ...
    for (const TS::OptionParsers::ParameterList::value_type &p :
         poptions.parameters()) {
      Precision::Map::const_iterator it =
          prec.parameter_precisions.find(p.name());
...
#endif
{
  try {
    // Get the option string
    string opt = Spine::required_string(theReq.getParameter("param"),
    ...
    if (theObsEngine != nullptr) {
      std::set<std::string> obsEngineStationTypes =
          theObsEngine->getValidStationTypes();
      for (const auto &areaproducers : timeproducers) {
        if (obsProducersExist)
          break;

        for (const auto &producer : areaproducers) {
          if (obsEngineStationTypes.find(producer) !=
              obsEngineStationTypes.end()) {
    ...
    for (const string &paramname : names) {
      try {
        TS::ParameterAndFunctions paramfuncs =
            TS::ParameterFactory::instance().parseNameAndFunctions(paramname,
                                                                   true);

        poptions.add(paramfuncs.parameter, paramfuncs.functions);
      } catch (...) {
        Fmi::Exception exception(
            BCP, "Parameter parsing failed for '" + paramname + "'!", NULL);
        throw exception;
      }
    }
    ...
    if (aggregationIntervalStringBehind.find(':') != string::npos) {
      aggregationIntervalStringAhead = aggregationIntervalStringBehind.substr(
          aggregationIntervalStringBehind.find(':') + 1);
      aggregationIntervalStringBehind = aggregationIntervalStringBehind.substr(
          0, aggregationIntervalStringBehind.find(':'));
    }

    int agg_interval_behind(
        Spine::duration_string_to_minutes(aggregationIntervalStringBehind));
    int agg_interval_ahead(
        Spine::duration_string_to_minutes(aggregationIntervalStringAhead));
    ...
    for (const TS::ParameterAndFunctions &paramfuncs :
      ...
      if (maxAggregationIntervals.find(paramname) ==
          maxAggregationIntervals.end())
        maxAggregationIntervals.insert(
            make_pair(paramname, AggregationInterval(0, 0)));

      if (paramfuncs.functions.innerFunction.type() ==
          TS::FunctionType::TimeFunction) {
