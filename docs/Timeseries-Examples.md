...
The examples use the `/timeseries` path. In an own installation the path is the
one configured with the `timeseries_url` attribute of the EDR plugin; if the
attribute is not defined at all, the timeseries interface is not available. See
[Configuration of the Timeseries Interface](Using-the-Timeseries-API.md#configuration-of-the-timeseries-interface).
