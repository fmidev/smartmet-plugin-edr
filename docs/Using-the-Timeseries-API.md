...
work. For example queries see the [Examples-page](Timeseries-Examples.md).

The timeseries interface described here is served by the EDR plugin as an
**optional** feature: it is available only if the `timeseries_url` attribute is
defined in the EDR plugin configuration file. See
[Configuration of the Timeseries Interface](#configuration-of-the-timeseries-interface)
for the details. The query syntax is the same as in the standalone
TimeSeries plugin the code was merged in from.
...
- [Requested Data Values (param)](#requested-data-values-param)
  ...
  - [Unique Parameter Keys](#unique-parameter-keys)
...
- [Functions](#functions)
  ...
  - [Grid-Engine Functions](#grid-engine-functions)
...
- [Configuration of the Timeseries Interface](#configuration-of-the-timeseries-interface)
  ...
  - [Enabling the timeseries interface](#enabling-the-timeseries-interface)
  - [Plugin configuration file](#plugin-configuration-file)

## Introduction

The timeseries interface offers a convenient way to fetch the meteorological time
...
The timeseries interface can fetch data related to 1) meteorological observations
...
these modules should be in place when using the timeseries interface.

It is good to realize that the timeseries interface queries meteorological data
...
to the timeseries interface. For example, the following request fetches the
...
The timeseries interface uses these parameters to call the QEngine or the ObsEngine
...
The timeseries interface returns a  HTTP error message such as "Bad Request",
...
forecasts. So the user when requesting the data using the timeseries interface
...
The timeseries interface can fetch data from several producers using a single request
...
id when  requesting the data for these areas using the timeseries interface. The
timeseries interface uses the location name or the id  to fetch the actual
...
which defines how wide the circular area is to the timeseries interface request.
...
requesting data from the timeseries interface. Examples are in parentheses.
...
At the moment, the timeseries interface does not directly support polygon
coordinates in the request. So one has to give the  area name and the
timeseries interface  fetches the area coordinates from the PostGIS database that
...
requesting data using the timeseries interface.
...
coordinates for a path is  technically viable but the timeseries interface uses
...
when requesting data using the timeseries interface:
...
The timeseries interface needs exact time period for querying data from the
QEngine/ObsEngine module. Usually this time period is given in the HTTP request
when the data is requested. If not, the timeseries interface uses the default
...
It is possible to  request the data fields (parameters) that the timeseries interface automatically generates based on the timestamp, the selected location,
...
The timeseries interface supports some aggregate  functions that can be  applied to
specific data fields of the queried data. As we have seen in the section on
[Areas](#areas),
when we request the temperature over a geographical area, the timeseries interface
...
the timeseries interface configuration file.
...
## Configuration of the Timeseries Interface

The timeseries interface is served by the EDR plugin and it is **optional**: it
exists only if the `timeseries_url` attribute is defined in the EDR plugin
configuration file. Everything else — engines, producers, parameter aliases,
precisions, geometry tables — is shared with the EDR API served by the same
plugin.

In order to use the timeseries interface the user needs to edit two
configuration files, namely the main configuration file of the SmartMet Server
environment and the configuration file of the EDR plugin.
...
plugins and engines are using. So, in order to use the timeseries interface you
need to do the following steps:

First check that there is an "edr" entry listed in the "plugins" attribute. If
not, add it. Note that no separate "timeseries" plugin entry is needed: the
timeseries interface is provided by the EDR plugin itself.

```text
plugins = ["edr"]
```

Second check that the "engines" attribute contains all the required engines such
as QEngine, GeoEngine, ObsEngine, and GIS. Notice that the "geoengine" entry must
...
Third check that there is a dedicated configuration entry for the EDR plugin. In
this entry you can disable or enable the usage of the plugin. You can also define
the name and path of the configuration file for the plugin.

```text
edr:
{
     disabled = false;
     configfile = "/etc/smartmet/plugins/edr.conf"
};
```

### Enabling the timeseries interface

The timeseries interface is enabled by defining the `timeseries_url` attribute in
the EDR plugin configuration file (`edr.conf`). If the attribute is missing, the
plugin serves the EDR API only and timeseries requests are not handled at all.

```text
url            = "/edr";        # EDR API
timeseries_url = "/timeseries"; # timeseries interface
```

Several URL paths can be served at the same time by giving an array of strings.
This is useful for example when clients are being migrated from an older path to
a new one:
...
The configured paths are canonicalized: a leading '/' is added if it is missing
and a trailing '/' is removed. A duplicate entry, a non-string array entry or a
value that is neither a string nor an array of strings is a configuration error
and prevents the plugin from starting.

Requests arriving to any of the configured paths are handled as timeseries
requests as described on this page. Requests arriving to the EDR `url` path are
handled according to the OGC API - EDR standard, see
[Using the EDR Plugin](Using-EDR-Plugin.md).

### Plugin configuration file

The table below describes the configuration attributes of `edr.conf` that affect
timeseries requests. Attributes that only affect the EDR API (collection
metadata, data queries, output formats, AVI collections, landing page templates,
etc.) are described in the
[EDR Plugin Configuration Guide](EDR-Plugin-Configuration-Guide.md).
...
<tr><td colspan="2"> url </td> <td>The URL path of the EDR API (e.g. "/edr")</td> </tr>
<tr><td colspan="2"> timeseries_url </td> <td>The URL path (e.g. "/timeseries") or an array of URL paths of the timeseries interface. If not defined, the timeseries interface is disabled.</td> </tr>
...
<tr><td colspan="2"> gridengine_disabled </td> <td> This attribute can be used to enable/disable the usage of the Grid-engine. It can have the values "true" or "false" </td></tr>
<tr><td colspan="2"> aviengine_disabled </td> <td> This attribute can be used to enable/disable the usage of the Avi-engine. It can have the values "true" or "false" </td></tr>
<tr><td colspan="2"> primaryForecastSource </td> <td> The engine used for forecast data when the producer does not determine it ("grid" or "querydata"). A "source" parameter in the request overrides this.</td></tr>
<tr><td colspan="2"> maxdistance</td> <td> The default maximum distance value for point forecasts (km)</td></tr>
<tr><td colspan="2"> maxradius</td> <td> The maximum radius (km) allowed in requests</td></tr>
<tr><td colspan="2"> expires</td> <td> The value used for the Expires and Cache-Control headers (seconds)</td></tr>
<tr><td colspan="2"> pretty</td> <td> Allow pretty printed output ("true" or "false")</td></tr>
<tr><td rowspan="5">request_limits </td> <td> maxlocations </td> <td> The maximum number of locations allowed in a request</td></tr>
<tr><td> maxparameters </td><td>The maximum number of parameters allowed in a request</td></tr>
<tr><td> maxtimes </td><td>The maximum number of time steps allowed in a request</td></tr>
<tr><td> maxlevels </td><td>The maximum number of levels allowed in a request</td></tr>
<tr><td> maxelements </td><td>The maximum number of result elements allowed in a request</td></tr>
<tr><td> cache </td> <td> timeseries_size </td> <td>The number of timeseries requests that are cached internally</td></tr>
<tr><td rowspan="3">wxml </td> <td>  timestring</td> <td> The default time format used with the WXML format (e.g. "%Y-%b-%dT%H:%M:%S")</td></tr>
...
<tr><td rowspan="2"> precision </td> <td> enabled </td> <td>A list of precision names that can be used with the "precision" parameter when requesting the data. Typically the following precision names are defined:  "normal", "double", and "full". The first name in the list is the default precision.
In addition to this, the meaning of these attributes are also defined on the parameter level later in the configuration file. In other words, the "normal" precision for the parameter X might be 2 decimals while the "normal" precision for the parameter Z might be 5 decimals. All the precision attributes are defined in the configuration file</td></tr>
<tr><td> enabled_timeseries </td><td>An optional list with the same contents as "enabled", used for timeseries requests only. Its first entry is the default precision of timeseries requests. If the attribute is not defined, the default of "enabled" is used for timeseries requests too. This makes it possible to keep the EDR API and the timeseries interface at their own default precisions (for example "auto" for EDR and "normal" for timeseries).</td></tr>
<tr><td rowspan="6">geometry_tables </td> <td>server</td> <td>The name of the PostGIS server as configured in the gis-engine. An empty value means the default server of the gis-engine.</td></tr>
<tr><td> schema </td><td>The default PostGIS schema name.</td></tr>
<tr><td> table </td><td>The default PostGIS table name.</td></tr>
<tr><td> field </td><td>The default PostGIS column name.</td></tr>
<tr><td> name </td><td>The default source name.</td></tr>
<tr><td> additional_tables </td><td>A list of additional geometry sources. Each entry may define name, server, schema, table and field; the default values above are used for the ones that are omitted.</td></tr>
</table>

**Note:** the geometry sources of the EDR plugin are configured with the
`geometry_tables` attribute. In the standalone TimeSeries plugin the
corresponding attribute was named `postgis`.
